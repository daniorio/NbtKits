...
public class kits extends JavaPlugin {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("kit") && args.length == 2 && sender instanceof DedicatedServer) {
			// /kit start daniorio
			try { InventoryNBTSer.getKit(Bukkit.getPlayer(args[1]), folder, args[0], true); } catch (IOException e) { }
			return true;
		}
		...
		if (cmd.getName().equalsIgnoreCase("kit")) {
			try {
				...
				if (args.length == 0) {
					...
					for(String s : InventoryNBTSer.getLogs(folder)) if(sender.hasPermission("kits.kit."+s)) l.add(s);
